...
namespace ManejoPresupuestos.Controllers
{
    public class TransaccionesController : Controller
    {
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuariosId();
            IEnumerable<ResultadoObtenerPorSemana> trasaccionesPorSemana = await servicioReportes.ObtenerReporteSemanal(usuarioId, mes, año, ViewBag);

            var agrupado = trasaccionesPorSemana.GroupBy(x=> x.Semana).Select(x=>new ResultadoObtenerPorSemana()
            {
                Semana = x.Key,
                Ingresos = x.Where(x => x.TipoOperacionesId == TipoOperacion.Ingreso).Select(x => x.Monto).FirstOrDefault(),
                Gastos = x.Where(x => x.TipoOperacionesId == TipoOperacion.Gasto).Select(x => x.Monto).FirstOrDefault()
            }).ToList();
